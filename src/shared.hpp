...
constexpr uint32_t inthash(uint32_t x)
{
    x ^= x >> 16;
    x *= 0x7feb352dU;
    x ^= x >> 15;
...
constexpr bool values_distinct(const Container& c)
{
...
template <size_t LutSize, typename Values>
constexpr uint32_t find_offset(const Values& vals)
{
    // 256 chosen arbitrarily until it doesn't work anymore
    for (uint32_t offset = 0; offset < 256; ++offset) {
        std::array<uint32_t, vals.size()> hashes;
        for (size_t i = 0; i < vals.size(); ++i) {
            hashes[i] = inthash(vals[i] + offset) % LutSize;
        }
        if (values_distinct(hashes)) {
            return offset;
        }
    }
    return -1;
...
template <typename Value, size_t NumValues, size_t LutSize>
struct PerfectHasher {
    constexpr PerfectHasher(std::array<Value, NumValues> values)
        : offset(find_offset<LutSize>(values))
    {
        lut.fill(NumValues);
        for (size_t i = 0; i < values.size(); ++i) {
            const auto h = inthash(values[i] + offset) % lut.size();
            assert(lut[h] == NumValues);
            lut[h] = i;
        }
    }

    constexpr uint32_t hash(uint32_t v) const { return lut[inthash(v + offset) % lut.size()]; }

    std::array<uint32_t, LutSize> lut = {};
    uint32_t offset;
};
